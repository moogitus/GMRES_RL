...
SAC training/evaluation script for the AK-SLRL paper-style continuous-action
environment (control_env.AKSLRLEnv). Single-life RL: a fresh SAC agent is
trained online during one GMRES(m) solve per matrix.

Mirrors train_dqn.py, with three differences:
  - uses SAC (continuous action) instead of DQN (discrete)
  - uses AKSLRLEnv (full residual observation, paper reward)
  - adds .mat loader for SuiteSparse files
...
def run_sac(A, b, args, seed):
    ...
    # AK-SLRL paper: replay buffer = half matrix size, capped at 20,000.
...
def main():
    ...
    parser.add_argument("--no-log-residual", dest="include_log_residual",
                        action="store_false")
    # SAC hyperparameters.
    #   Paper-specified (Keramati & Hamdullahpur 2025, sec. 3):
    #     gamma = 0.97
    #     buffer_size = min(n // 2, 20_000)   ("short replay buffer")
    #     ent_coef = 'auto'                   ("high rate of exploration at the beginning")
    #   Not specified by the paper -> use stable_baselines3 SAC defaults:
    #     learning_rate = 3e-4
    #     learning_starts = 100
    #     batch_size = 256
    #     tau = 0.005
    ...
    parser.add_argument("--ent-coef", type=str, default="auto",
                        help="Paper: 'auto' (yields the high initial exploration described in sec. 3).")
    ...
    parser.add_argument("--out", type=str, default="logs/train_sac_collection.json")
    args = parser.parse_args()

    # Allow string-or-float ent_coef without making argparse choke on 'auto'.
    try:
